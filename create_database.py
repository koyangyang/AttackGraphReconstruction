...
def create_node_list(cur, connect):
    ...
    file_uuid2hash = {}
    for i in records:
        file_uuid2hash[i[0]] = i[1]  # "hash_value": "uuid"
            ...
            for line in f:
                if '{"datum":{"com.bbn.tc.schema.avro.cdm18.Event"' in line and "EVENT_FLOWS_TO" not in line:
                    ...
                    if len(subject_uuid) > 0 and len(predicateObject_uuid) > 0:
                        subject_uuid = subject_uuid[0].lower()  # 最坑代码
                        predicateObject_uuid = predicateObject_uuid[0].lower()  # 最坑代码
                        if subject_uuid in subject_uuid2hash and (
                                predicateObject_uuid in file_uuid2hash or predicateObject_uuid in net_uuid2hash):
